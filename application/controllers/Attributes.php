...
class Attributes extends Admin_Controller 
{
	public function __construct()
	{
		...
		$this->data['page_title'] = 'Quản Lý Thuộc Tính';
	...
	*/
	public function fetchAttributeData()
	{
		...
		foreach ($data as $key => $value) {
			...
			<button type="button" class="btn btn-default" onclick="editFunc('.$value['id'].')" data-toggle="modal" data-target="#editModal"><i class="fa fa-pencil"></i> Sửa</button>
			<button type="button" class="btn btn-default" onclick="removeFunc('.$value['id'].')" data-toggle="modal" data-target="#removeModal"><i class="fa fa-trash"></i> Xóa</button>
			';
	...
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	if($create == true) {
        		...
        		$response['messages'] = 'Lỗi trong cơ sở dữ liệu khi tạo thông tin thông tin';			
        	}
        }
        else {
        	$response['success'] = false;
	...
	public function remove()
	{
		...
		if($attribute_id) {
			...
			if($delete == true) {
				...
				$response['success'] = false;
				$response['messages'] = "Lỗi trong cơ sở dữ liệu trong khi xóa thông tin thương hiệu";
			}
		}
		else {
			$response['success'] = false;
	...
	*/
	public function fetchAttributeValueData($attribute_parent_id)
	{
		...
		foreach ($data as $key => $value) {
			...
			// button
			$buttons = '
			<button type="button" class="btn btn-default" onclick="editFunc('.$value['id'].')" data-toggle="modal" data-target="#editModal"><i class="fa fa-pencil"> Sửa</i></button>
			<button type="button" class="btn btn-default" onclick="removeFunc('.$value['id'].')" data-toggle="modal" data-target="#removeModal"><i class="fa fa-trash"> Xóa</i></button>
			';
